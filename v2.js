var data = [
  {
    id: 1,
    title: "Saibo",
    videoID: "t-NST0awBw8",
  },
  {
    id: 2,
    title: "Kahin toh hogi woh",
    videoID: "bF_ZKgZE0EU",
  },
  {
    id: 3,
    title: "Tune jo na kaha",
    videoID: "W9AEvho-kew",
  },
  {
    id: 4,
    title: "Peheli nazar",
    videoID: "Kv3NrD5h0Vk",
  },
  {
    id: 5,
    title: "Kya mujhe pyaar hai",
    videoID: "qi6fOkfdkgg",
  },
  {
    id: 6,
    title: "Tu chaiye",
    videoID: "X5s5DbXM4Xg",
  },
  {
    id: 7,
    title: "Zindagi kuch toh Bata",
    videoID: "cFqQi6fYm78",
  },
  {
    id: 8,
    title: "Jaan Nisar",
    videoID: "ILgojCXLyA4",
  },
  {
    id: 9,
    title: "Qaafirana",
    videoID: "bXC2AQt4Qwo",
  },
  {
    id: 10,
    title: "Phir Kabhi",
    videoID: "dJGGc3OgdrI",
  },
  {
    id: 11,
    title: "Chaap Tilak",
    videoID: "weUTe1M_wxY",
  },
  {
    id: 12,
    title: "Woh lamhe woh baatein",
    videoID: "_2bcwS9Wlb8",
  },
  {
    id: 13,
    title: "Mann mera",
    videoID: "tG4Rq192WW0",
  },
  {
    id: 14,
    title: "Kaun tujhe",
    videoID: "uXjAKbY9Q7E",
  },
  {
    id: 15,
    title: "Kuchh toh hai",
    videoID: "BAUW5gPneV4",
  },
  {
    id: 16,
    title: "Kho Gaye hum kahan",
    videoID: "QS5lVpVnd2o",
  },
  {
    id: 17,
    title: "Iktara",
    videoID: "PI-LIJ1RXEc",
  },
  {
    id: 18,
    title: "Jo bheji thi duaa",
    videoID: "hIhwaZn456U",
  },
  {
    id: 19,
    title: "Kun faya kun",
    videoID: "nynVielA6ZU",
  },
  {
    id: 20,
    title: "Jadu hai nasha hai",
    videoID: "ce20nljBdQ8",
  },
  {
    id: 21,
    title: "Noor e Khuda",
    videoID: "_jpfs-qohRU",
  },
  {
    id: 22,
    title: "Channa Mereya",
    videoID: "nGf_VfxpLNY",
  },
  {
    id: 23,
    title: "Ranjhana",
    videoID: "UomCE-0BbSk",
  },
  {
    id: 24,
    title: "Tu hai",
    videoID: "8p_T09VXNFk",
  },
  {
    id: 25,
    title: "Aayat",
    videoID: "7PGzTD3vA_k",
  },
  {
    id: 26,
    title: "Dildara",
    videoID: "oGo6HRTZ78Q",
  },
  {
    id: 27,
    title: "Jashne bahara",
    videoID: "rbCGmC1J1x8",
  },
  {
    id: 28,
    title: "Tum se hi",
    videoID: "mOAQLgOue5I",
  },
  {
    id: 29,
    title: "Hey ya",
    videoID: "wWZlY5q67RA",
  },
  {
    id: 30,
    title: "Ye Dooriyaan",
    videoID: "G76FXVoHyQ0",
  },
  {
    id: 31,
    title: "Sajde kiye hai",
    videoID: "ZToOHtiOWMg",
  },
  {
    id: 32,
    title: "Tum ho",
    videoID: "OCi6Vay-J7g",
  },
  {
    id: 33,
    title: "Main Hoon Saath Tere",
    videoID: "IgEmHC7t2qg",
  },
  {
    id: 34,
    title: "Agar tum Saath ho",
    videoID: "G8Cddg5M7eA",
  },
  {
    id: 35,
    title: "Moh Moh Dhage",
    videoID: "2-YmyAm__II",
  },
  {
    id: 36,
    title: "Jeena Jeena",
    videoID: "7cyUoUsBI1c",
  },
  {
    id: 37,
    title: "Juda hoke bhi",
    videoID: "sTsV5m_oV7E",
  },
  {
    id: 38,
    title: "Humdard",
    videoID: "sWBKYyh2N7c",
  },
  {
    id: 39,
    title: "Mast Magan",
    videoID: "5ghEXrWWqzI",
  },
  {
    id: 40,
    title: "Kyun",
    videoID: "svVXLUt5SV8",
  },
  {
    id: 41,
    title: "Jiyein kyun",
    videoID: "b5Yy1qR2PQc",
  },
  {
    id: 42,
    title: "Maula Mere Maula",
    videoID: "-ChbVYKRC-k",
  },
  {
    id: 43,
    title: "Tere bina",
    videoID: "KwviqJMwUP8",
  },
  {
    id: 44,
    title: "Tere naina",
    videoID: "3Ej1hvhkuvs",
  },
  {
    id: 45,
    title: "Teri jhuki nazar",
    videoID: "JXgBXGql7Gg",
  },
  {
    id: 46,
    title: "Labon ko",
    videoID: "7GM-3NKJpVY",
  },
  {
    id: 47,
    title: "Beetein Lamhein",
    videoID: "8E3F8kFnyPU",
  },
  {
    id: 48,
    title: "Pal",
    videoID: "SIqnvMt8IMU",
  },
  {
    id: 49,
    title: "Mere Yaaraa",
    videoID: "pENZaYQMSL0",
  },
  {
    id: 50,
    title: "Main agar kahoon",
    videoID: "YzdzV5zuDHc",
  },
  {
    id: 51,
    title: "Ishq sufiyana",
    videoID: "G3GtUvmu_VI",
  },
  {
    id: 52,
    title: "Ore piya",
    videoID: "37pnbV4mViY",
  },
  {
    id: 53,
    title: "In dino",
    videoID: "Hn6_hEKORXY",
  },
  {
    id: 54,
    title: "Saiyan",
    videoID: "vkBTEvxVImQ",
  },
  {
    id: 55,
    title: "Tum mile",
    videoID: "yJQOfXhe9-4",
  },
  {
    id: 56,
    title: "Wada raha",
    videoID: "Y-0P9kQFp0U",
  },
  {
    id: 57,
    title: "Zara Zara Bahekta Hain",
    videoID: "QASQK09S5ek",
  },
  {
    id: 58,
    title: "Aankhon mein teri",
    videoID: "nI8NUDXsYTo",
  },
  {
    id: 59,
    title: "Sapna jahan",
    videoID: "uhBZ_FQWp9I",
  },
  {
    id: 60,
    title: "Main hoon na",
    videoID: "yjfQUxKYrpo",
  },
  {
    id: 61,
    title: "Shayad",
    videoID: "sTI9zWoDCWA",
  },
  {
    id: 62,
    title: "Kaise mujhe",
    videoID: "2J9I8jmiPEs",
  },
  {
    id: 63,
    title: "Pehla Nasha",
    videoID: "PZ7mhXZSJ8c",
  },
  {
    id: 64,
    title: "Baarishein",
    videoID: "UZVIWGfKb68",
  },
  {
    id: 65,
    title: "Dil kyun ye mera",
    videoID: "Q1PXUzEMh6c",
  },
  {
    id: 66,
    title: "Guzarish",
    videoID: "c-nnPZcJcdQ",
  },
  {
    id: 67,
    title: "Tum tak",
    videoID: "DfTXRfOBY-k",
  },
  {
    id: 68,
    title: "Jeene laga hun",
    videoID: "4P7c8LRO0DU",
  },
  {
    id: 69,
    title: "Zaalima",
    videoID: "tDzg16gJr0s",
  },
  {
    id: 70,
    title: "Dekhte dekhte",
    videoID: "eZHaumDApl0",
  },
  {
    id: 71,
    title: "O saathi tere bina",
    videoID: "P2GPHCHXxXI",
  },
  {
    id: 72,
    title: "Tera hua",
    videoID: "cZKyUIhtV4A",
  },
  {
    id: 73,
    title: "Dil diyan gallan",
    videoID: "VDzjgO7-pVI",
  },
  {
    id: 74,
    title: "Sajdaa",
    videoID: "L3R1wDLQZK0",
  },
  {
    id: 75,
    title: "O re piya",
    videoID: "1HuMgUm1Yxg",
  },
  {
    id: 76,
    title: "Laal ishq",
    videoID: "1BD1ETuQi3k",
  },
  {
    id: 77,
    title: "Phir le aya dil",
    videoID: "TYYyb3TFNOA",
  },
  {
    id: 78,
    title: "Hawayein",
    videoID: "BjrFhePFaMc",
  },
  {
    id: 79,
    title: "Tera chehra",
    videoID: "nCHhsG4nckM",
  },
  {
    id: 80,
    title: "Janam janam",
    videoID: "bOj8c8JO-FQ",
  },
  {
    id: 81,
    title: "Phir bhi tumko chahunga",
    videoID: "UDUSE5tr2aU",
  },
  {
    id: 82,
    title: "Ae dil hai mushkil ",
    videoID: "cAlwHW-FTVs",
  },
  {
    id: 83,
    title: "Chahun mein ya na",
    videoID: "ob1pD8v0_cE",
  },
  {
    id: 84,
    title: "Tum hi ho",
    videoID: "cIGd0U2QzB0",
  },
  {
    id: 85,
    title: "Tera fitoor",
    videoID: "X7qUilTvvXg",
  },
  {
    id: 86,
    title: "Samjhawan",
    videoID: "c7bfJARNpi8",
  },
  {
    id: 87,
    title: "Shukran allah",
    videoID: "eLddmZqaf7Q",
  },
  {
    id: 88,
    title: "Tere liye",
    videoID: "otq-SQ3Yrjg",
  },
  {
    id: 89,
    title: "Kaun tujhe",
    videoID: "OO3BWOKnZa8",
  },
  {
    id: 90,
    title: "Main yahan hoon ",
    videoID: "ryGBC-y_OPo",
  },
  {
    id: 91,
    title: "Ye haseen wadiyan",
    videoID: "QhwtMqzZ4R0",
  },
  {
    id: 92,
    title: "Tauba tumhare",
    videoID: "DXKcYu7DsC0",
  },
  {
    id: 93,
    title: "Bhage re maan kahin",
    videoID: "vZFzMvKqZRA",
  },
  {
    id: 94,
    title: "Ye fitoor mera",
    videoID: "IbGV-ggNU-0",
  },
  {
    id: 95,
    title: "Khuda jane",
    videoID: "Jho5jDYOcXw",
  },
  {
    id: 96,
    title: "Suno na Suno na",
    videoID: "E4ciH0HvOvE",
  },
  {
    id: 97,
    title: "Tumhi dekho na",
    videoID: "4_sQisHGKLw",
  },
  {
    id: 98,
    title: "Kabira",
    videoID: "aXs_CMqXfxM",
  },
  {
    id: 99,
    title: "Na tum jano na hum",
    videoID: "r87kfxXqyoQ",
  },
  {
    id: 100,
    title: "Lag ja gale",
    videoID: "KuUTC_BqCQU",
  },
  {
    id: 101,
    title: "Pihu bole",
    videoID: "aoWrc4T8neM",
  },
  {
    id: 102,
    title: "Bepanh pyaar hai",
    videoID: "XNHacpmIUlQ",
  },
  {
    id: 103,
    title: "Tum jo aaye",
    videoID: "azHbsWaJDUc",
  },
  {
    id: 104,
    title: "Chaudhary",
    videoID: "rRjFG5bVOCA",
  },
  {
    id: 105,
    title: "Kabhi alvida na kehna",
    videoID: "MFzd7YLHCy4",
  },
  {
    id: 106,
    title: "Pani daa",
    videoID: "jRgcExSYrtA",
  },
  {
    id: 107,
    title: "Phir se ud chala",
    videoID: "hUk1FDafJ4s",
  },
  {
    id: 108,
    title: "Nain Na Jodeen",
    videoID: "qDZgOmvT-Dk",
  },
  {
    id: 109,
    title: "Phir Na Milen Kabhi",
    videoID: "2V9MZhBHbpk",
  },
  {
    id: 110,
    title: "Bheegi Si Bhaagi Si",
    videoID: "HwiddDeSDtA",
  },
  {
    id: 111,
    title: "Main kya karun",
    videoID: "PS2sGa7w9W8",
  },
  {
    id: 112,
    title: "Mann Mera",
    videoID: "3CCNOmMjj6E",
  },
  {
    id: 113,
    title: "Dil",
    videoID: "E_uuYsHXyro",
  },
  {
    id: 114,
    title: "Saadi Galli Aaja",
    videoID: "T95ib7L3KjM",
  },
  {
    id: 115,
    title: "Tu Banja Gali Benaras Ki",
    videoID: "4z46RWw_Kyo",
  },
  {
    id: 116,
    title: "Jogi",
    videoID: "TK82vxFzAA8",
  },
  {
    id: 117,
    title: "Hoor",
    videoID: "KTGOTX03Ny0",
  },
  {
    id: 118,
    title: "Nadaan Parindey",
    videoID: "rKUZnrPsu2Y",
  },
  {
    id: 119,
    title: "Thoda thoda pyar hua",
    videoID: "BzI1MZnFFqQ",
  },
  {
    id: 120,
    title: "Hoshwalon Ko Khabar Kya",
    videoID: "_6fZla-78gc",
  },
  {
    id: 121,
    title: "Lae Dooba",
    videoID: "En0n_EEbYiA",
  },
  {
    id: 122,
    title: "Judaai",
    videoID: "qoEGSjsrJEc",
  },
  {
    id: 123,
    title: "Pee Loon",
    videoID: "iLyE6e8gJx4",
  },
  {
    id: 124,
    title: "Lambi Judai",
    videoID: "Uk_i_RdZEqo",
  },
  {
    id: 125,
    title: "Haal e Dil",
    videoID: "aHYItdsAdL4",
  },
  {
    id: 126,
    title: "Mere samne wali khidki mein",
    videoID: "nSGDYYf9f-s",
  },
  {
    id: 127,
    title: "Gazab ka hai ye din",
    videoID: "b5w1gCEIN_w",
  },
  {
    id: 128,
    title: "Jaan ban gaye",
    videoID: "iSzo6JnHkGE",
  },
  {
    id: 129,
    title: "Dil Ibaadat",
    videoID: "OhUBlTVnE9k",
  },
  {
    id: 130,
    title: "Aadha Ishq",
    videoID: "cJhq4HVm_l4",
  },
  {
    id: 131,
    title: "Ishq de fanniyar",
    videoID: "Bpd9w8cmEuQ",
  },
  {
    id: 132,
    title: "Aaj kal zindagi",
    videoID: "p2PKvemHpJ0",
  },
  {
    id: 133,
    title: "Aaj din chadheya",
    videoID: "cE_QuqBqbRg",
  },
  {
    id: 134,
    title: "Zindagi do pal ki",
    videoID: "znXt1X6Hv3g",
  },
  {
    id: 135,
    title: "Kaisi Hai Yeh",
    videoID: "NHiPRHZlCro",
  },
  {
    id: 136,
    title: "Kuchh is tarah",
    videoID: "rvFsIrMLAC4",
  },
  {
    id: 137,
    title: "Pasoori",
    videoID: "kXTpt5g-AdA",
  },
  {
    id: 138,
    title: "Maahi Ve",
    videoID: "kBE2rlrJnbU",
  },
  {
    id: 139,
    title: "Rehna tu",
    videoID: "Qsidpk9DtYg",
  },
  {
    id: 140,
    title: "Matargashti",
    videoID: "rs4QMg_FB5Q",
  },
  {
    id: 141,
    title: "Nahin Samne",
    videoID: "wLxufx5SHsM",
  },
  {
    id: 142,
    title: "Mitwa",
    videoID: "KzpDCsrl1aI",
  },
  {
    id: 143,
    title: "kabhi yadon mein aao",
    videoID: "AEPZZpj7XiU",
  },
  {
    id: 144,
    title: "Dekh Lena",
    videoID: "qXxnk9_cEVI",
  },
  {
    id: 145,
    title: "Dil ko karar aya",
    videoID: "Ya25t4B9xTE",
  },
  {
    id: 140,
    title: "Masakali",
    videoID: "AmVZSBOrhCI",
  },
  {
    id: 141,
    title: "Ambarsariya",
    videoID: "GpPldiqQo8M",
  },
  {
    id: 142,
    title: "Tumko dekha to",
    videoID: "iZNPEXWlH4o",
  },
  {
    id: 143,
    title: "Baade achhe lagte hai",
    videoID: "tIzDSYH81B0",
  },
  {
    id: 144,
    title: "O Sanam",
    videoID: "5p-E1ZGHZFA",
  },
  {
    id: 145,
    title: "Yeh jo des hai tera",
    videoID: "yL4CqKuzMmQ",
  },
  {
    id: 146,
    title: "Roshni se",
    videoID: "xJ9j-KF5BcY",
  },
  {
    id: 147,
    title: "Saans",
    videoID: "ywz6qSiynf0",
  },
  {
    id: 148,
    title: "Namo namo",
    videoID: "OrGfBZYWV30",
  },
  {
    id: 149,
    title: "Darkhaast",
    videoID: "vE6xHY2dHJM",
  },
  {
    id: 150,
    title: "O meri jaan",
    videoID: "fOWXZCany1w",
  },
  {
    id: 151,
    title: "Mere khayalon ki malika",
    videoID: "5A1WYZQG1M4",
  },
  {
    id: 152,
    title: "Tumhi ho bandhu",
    videoID: "30F6q4HHbeA",
  },
  {
    id: 153,
    title: "Sun sathiya",
    videoID: "fw17I6sf-8c",
  },
  {
    id: 154,
    title: "Ghalat Fehmi",
    videoID: "VE0l8uXZztg",
  },
  {
    id: 155,
    title: "muskaanein jhooti hai",
    videoID: "XkiChjtOrVw",
  },
  {
    id: 156,
    title: "Zinda hoon yaar",
    videoID: "VIWTu1SQv04 ",
  },
  {
    id: 157,
    title: "Tujhe bhula Diya",
    videoID: "msW7Vk6zdXs",
  },
  {
    id: 158,
    title: "Humnava",
    videoID: "2U5-53WAdnM",
  },
  {
    id: 159,
    title: "Roke na ruke naina",
    videoID: "QQK23RRUcYY",
  },
  {
    id: 160,
    title: "Humari Aadhuri Kahani",
    videoID: "ruB9D5NM1JI",
  },
  {
    id: 161,
    title: "Sawar Loon",
    videoID: "HNjRnvr_iUw",
  },
  {
    id: 162,
    title: "Jhumritalaiyya",
    videoID: "K2iUzYFtvKk",
  },
  {
    id: 163,
    title: "Jee Le Zara",
    videoID: "iri2jqjKxow",
  },
  {
    id: 164,
    title: "Aadat",
    videoID: "jFImbhU7BL8",
  },
  {
    id: 165,
    title: "Khaabon Ke Parinday",
    videoID: "5mQL5MnUKaE",
  },
  {
    id: 167,
    title: "Der Lagi Lekin",
    videoID: "jYnd-qawF5E",
  },
  {
    id: 168,
    title: "Jo Bhi Main",
    videoID: "yWgosIRwjuM",
  },
  {
    id: 169,
    title: "Anjana Anjani",
    videoID: "km8gpaMMd04",
  },
  {
    id: 170,
    title: "Tera Ban Jaunga",
    videoID: "GedwWPSO0u0",
  },
  {
    id: 171,
    title: "Kaise Hua",
    videoID: "pybWGv0uSRY",
  },
  {
    id: 172,
    title: "Luka Chuppi",
    videoID: "DGYm_UbAQcw",
  },
  {
    id: 173,
    title: "Bandeyaa",
    videoID: "jrixXpaW8BQ",
  },
  {
    id: 174,
    title: "Pyaar Ki Ek Kahani",
    videoID: "DIi2Q3IlWFQ",
  },
  {
    id: 175,
    title: "Heeriye",
    videoID: "Xwq8lRX8GQA",
  },
  {
    id: 176,
    title: "koi tumsa nahin",
    videoID: "Jz7th1J9z0U",
  },
  {
    id: 177,
    title: "Tum Ho Toh ",
    videoID: "BHWLR1UdDOk",
  },
  {
    id: 178,
    title: "Tera Mera Rishta ",
    videoID: "xgYkQOTwq74",
  },
  {
    id: 179,
    title: "Teri Ore",
    videoID: "NQTTgj7XSDQ",
  },
  {
    id: 180,
    title: "Teri Mitti ",
    videoID: "irlaQygjevE",
  },
  {
    id: 181,
    title: "Gilehriyaan",
    videoID: "DGUL_17mKtc",
  },
  {
    id: 182,
    title: "Besabriyaan",
    videoID: "SYiPWn0tWLQ",
  },
  {
    id: 183,
    title: "Piya Basanti ",
    videoID: "nXHB5iuEb2I",
  },
  {
    id: 184,
    title: "Toh Phir Aao",
    videoID: "r8_axTpja78",
  },
  {
    id: 185,
    title: "Tu hi re",
    videoID: "Ver07mb5NX4",
  },
  {
    id: 186,
    title: "badi mushkil hai ",
    videoID: "ql8szL426o0",
  },
  {
    id: 187,
    title: "Sham",
    videoID: "MAHtfknx75g",
  },
  {
    id: 188,
    title: "Suno Na Sangemarmar",
    videoID: "3HX6WCx2djU",
  },
  {
    id: 189,
    title: "Aa bhi ja",
    videoID: "mA0WzUoHlrI",
  },
  {
    id: 190,
    title: "Mein dhundne ko",
    videoID: "N_857osuvWw",
  },
  {
    id: 191,
    title: "Saathi Rey",
    videoID: "Fr_KuJDepdQ",
  },
  {
    id: 192,
    title: "Iktara",
    videoID: "nJcaU8bKpGs",
  },
  {
    id: 193,
    title: "Hosanna",
    videoID: "BkGcOQ-jcZ8",
  },
  {
    id: 194,
    title: "Mehrama",
    videoID: "DNYd6CIRqaw",
  },
  {
    id: 195,
    title: "Yeh Dooriyan",
    videoID: "faj7jA-RSRg",
  },
  {
    id: 196,
    title: "Jashn-E-Bahaaraa",
    videoID: "rbCGmC1J1x8",
  },
  {
    id: 197,
    title: "Waalian",
    videoID: "zgqA5LKjoXI",
  },
  {
    id: 198,
    title: "Ek Dil Ek Jaan",
    videoID: "7wU2CsI7S_E",
  },
  {
    id: 199,
    title: "Phir Mohabbat",
    videoID: "NTsZuE8McPs",
  },
  {
    id: 200,
    title: "khuda hafiz",
    videoID: "Lz1v3EMvHDU",
  },
  {
    id: 201,
    title: "Chidiya",
    videoID: "LzxZ_ivoplo",
  },
  {
    id: 202,
    title: "Zara Tehro",
    videoID: "RGyv5Bq3YSw",
  },
  {
    id: 203,
    title: "Kal ho na ho",
    videoID: "AFKl9OaHn2w",
  },
  {
    id: 204,
    title: "Main Rang Sarbaton Ka",
    videoID: "YIxOzQ2NNEM",
  },
  {
    id: 206,
    title: "Meherbani",
    videoID: "LId--aOjZnU",
  },
  {
    id: 207,
    title: "Saware",
    videoID: "xqvCHx3KXM0",
  },
  {
    id: 208,
    title: "Bewajah",
    videoID: "TKz7qzbY-X0",
  },
  {
    id: 209,
    title: "Jannat",
    videoID: "gVodGPcnxeY",
  },
  {
    id: 210,
    title: "Bewafa",
    videoID: "9AlFM44TDtM",
  },
  {
    id: 211,
    title: "Jag Soona soona lage",
    videoID: "P4CdKL8TKQI",
  },
  {
    id: 212,
    title: "Ishq Mubarak",
    videoID: "9PYxQ85ORqE",
  },
  {
    id: 213,
    title: "Tujh Me Rab Dikhta Hai",
    videoID: "pbgrK6QRX4U",
  },
  {
    id: 214,
    title: "Gerua",
    videoID: "IWhyUgZuY9U",
  },
  {
    id: 215,
    title: "Tum hi Aana",
    videoID: "dqe8qKptzjM",
  },
  {
    id: 216,
    title: "Ranjha",
    videoID: "UTIg3LaDkSQ",
  },
  {
    id: 217,
    title: "Kabhi Tumhe",
    videoID: "kN2hp-vgkAc",
  },
  {
    id: 218,
    title: "Tu Chale",
    videoID: "xrT4w9R4SLY",
  },
  {
    id: 219,
    title: "Sajde Kiye Hai Lakho",
    videoID: "ZToOHtiOWMg",
  },
  {
    id: 219,
    title: "Meri Aashiqui",
    videoID: "DSC2b4BoMJU",
  },
  {
    id: 220,
    title: "Bol Do Na Zara",
    videoID: "z_xZMSC-mCk",
  },
  {
    id: 221,
    title: "Tera Naam Doon",
    videoID: "NiW3Kw0UWWg",
  },
  {
    id: 222,
    title: "Hum Mar Jayenge",
    videoID: "uvmw3K0p3Hk",
  },
  {
    id: 223,
    title: "Itni si Baat Hai",
    videoID: "5Tj8yXuzGjM",
  },
  {
    id: 224,
    title: "Ijazat",
    videoID: "Ae5ivrnHcAc",
  },
  {
    id: 225,
    title: "Thodi Der",
    videoID: "_pWeQoQuNFc",
  },
  {
    id: 226,
    title: "Kabhi Jo Badal Barse",
    videoID: "MgHL7WxcXSE",
  },
  {
    id: 227,
    title: "Chashni",
    videoID: "CiREtGAG75U",
  },
  {
    id: 228,
    title: "Sanu Ek Pal Chain",
    videoID: "2Bkn4X1_-gs",
  },
  {
    id: 229,
    title: "Surili Akhiyon Wale",
    videoID: "G2LgcXGei9I",
  },
  {
    id: 230,
    title: "Gazab Ka Hai Din",
    videoID: "b5w1gCEIN_w",
  },
  {
    id: 231,
    title: "Chale Aana",
    videoID: "JvkFJ8yjTGM",
  },
  {
    id: 232,
    title: "Kabhi to pass mere aao",
    videoID: "0t3WxzxcUEk",
  },
  {
    id: 233,
    title: "Baatein Ye Kaabhi Naa",
    videoID: "KfoF45jYu74",
  },
  {
    id: 234,
    title: "Dhal Jaun Main",
    videoID: "UboqYZR4K40",
  },
  {
    id: 235,
    title: "Dekha Hazaro Dafa Aapko",
    videoID: "rXy-AOxlkuc",
  },
  {
    id: 236,
    title: "Zaroori Tha",
    videoID: "9y1VMrIE3yc",
  },
  {
    id: 237,
    title: "Saiyaara",
    videoID: "oEYyaVaBBYg",
  },
  {
    id: 238,
    title: "Salamat",
    videoID: "dBHenPVKg7c",
  },
  {
    id: 239,
    title: "Tose Naina",
    videoID: "fY4NmyENzD4",
  },
  {
    id: 240,
    title: "Mujhe Kaise Pata Na Chala",
    videoID: "3mGr2C71MC8",
  },
  {
    id: 241,
    title: "Kabhi Kabhi Aditi",
    videoID: "UDjwaw78m-k",
  },
  {
    id: 242,
    title: "Excuses",
    videoID: "zMhwHZsHlJ4",
  },
  {
    id: 243,
    title: "Insane",
    videoID: "QgaZNXQkoos",
  },
  {
    id: 244,
    title: "Chand Sifarish",
    videoID: "aqg-Uugxz3Q",
  },
  {
    id: 245,
    title: "Suna Hai",
    videoID: "DI75anmn3gs",
  },
  {
    id: 246,
    title: "Ek Raat",
    videoID: "ag5n58osgAo",
  },
  {
    id: 247,
    title: "Mera Yaraa",
    videoID: "CvNGUiKobr4",
  },
  {
    id: 248,
    title: "Kinna Sona",
    videoID: "SReSUol49P4",
  },
  {
    id: 249,
    title: "Subhanallah",
    videoID: "ycPWEG_WB2U",
  },
  {
    id: 250,
    title: "Falak Tak",
    videoID: "aNthTpwyP-I",
  },
  {
    id: 251,
    title: "Pehla Nasha",
    videoID: "NEfopUdk4T4",
  },
  {
    id: 252,
    title: "Ratan Lambiyan",
    videoID: "jSvvb_5MDUI",
  },
  {
    id: 253,
    title: "Yeh Sham Mastani",
    videoID: "I3fUdvD4rUo",
  },
  {
    id: 254,
    title: "Tum kyun Chale aate ho",
    videoID: "BtXAMvjn-K0",
  },
  {
    id: 255,
    title: "Ishq Bulava",
    videoID: "m5MdVhG_B24",
  },
  {
    id: 256,
    title: "Ishq Bulava",
    videoID: "RvbV5IQBEmQ",
  },
  {
    id: 257,
    title: "Hum Toh Udd Gaye",
    videoID: "cOb63w8zCEE",
  },
  {
    id: 258,
    title: "Tera Hone Laga Hoo",
    videoID: "NN0OCn7vgfA",
  },
  {
    id: 259,
    title: "Aawari",
    videoID: "-W_m1YPos8Y",
  },
  {
    id: 260,
    title: "Shaam Gulabi",
    videoID: "J6ct0FL4TGY",
  },
  {
    id: 261,
    title: "Mat Aazma re",
    videoID: "2sAtiDQnnE8",
  },
  {
    id: 262,
    title: "Zara Sa",
    videoID: "Mx5NvDVXjU4",
  },
  {
    id: 263,
    title: "Tu Jo Mila",
    videoID: "oImOOl7weAc",
  },
  {
    id: 264,
    title: "Piya aaye na",
    videoID: "xURhIckKl7E",
  },
  {
    id: 267,
    title: "Labon ko",
    videoID: "7GM-3NKJpVY",
  },
  {
    id: 268,
    title: "Tu Hi Meri Shab Hai",
    videoID: "7kEjL1a-yDo",
  },
  {
    id: 269,
    title: "Abhi Abhi",
    videoID: "YKp37DAHDgE",
  },
  {
    id: 270,
    title: "Tum ho mera pyaar",
    videoID: "9ADJ-1ZClaw",
  },
  {
    id: 271,
    title: "Saanson Ke",
    videoID: "IjkH3ibpKTY",
  },
  {
    id: 272,
    title: "Aashayein",
    videoID: "Lv9j1_kzlTA",
  },
  {
    id: 273,
    title: "Humko Pyaar hua",
    videoID: "Xq9me3-Ju9c",
  },
  {
    id: 274,
    title: "o meri jaan",
    videoID: "c72CvtM4Z2A",
  },
  {
    id: 275,
    title: "Jannatein Kahan",
    videoID: "hxRpQSk1jh8",
  },
  {
    id: 276,
    title: "Mere Bina",
    videoID: "laUUf6PLxZU",
  },
  {
    id: 277,
    title: "Alvida",
    videoID: "o7rW1Vu0VTo",
  },
  {
    id: 278,
    title: "Aye Bekhabar",
    videoID: "hB7Fcc70G7s",
  },
  {
    id: 279,
    title: "Allah Hafiz",
    videoID: "FLeyoHdffLM",
  },
  {
    id: 280,
    title: "Dil Aaj Kal",
    videoID: "3y2QeUSdTGs",
  },
  {
    id: 281,
    title: "Pyar Ke Pal",
    videoID: "7SUxjc52-m8",
  },
  {
    id: 282,
    title: "Zindagi do pal ki",
    videoID: "znXt1X6Hv3g",
  },
  {
    id: 283,
    title: "Kaise Bataun Tujhe",
    videoID: "DKP_K9mC1d0",
  },
  {
    id: 284,
    title: "Hothon Se Chhu Lo Tum",
    videoID: "JWPMUAilm8I",
  },
  {
    id: 285,
    title: "Chhod Diya",
    videoID: "-tSkLXFbLA0",
  },
  {
    id: 286,
    title: "Galliyan Returns",
    videoID: "VX0Q4vxIm_U",
  },
  {
    id: 287,
    title: "Baarishon Mein",
    videoID: "89u-bWiNT74",
  },
  {
    id: 288,
    title: "Kesariya",
    videoID: "fqZJY-eVsio",
  },
  {
    id: 289,
    title: "Teri Meri Prem Kahani",
    videoID: "x_eDAsgFf1Y",
  },
  {
    id: 290,
    title: "Raba Wangu",
    videoID: "87nfrU6E2bE",
  },
  {
    id: 291,
    title: "Soch Liya",
    videoID: "jspsBXPBor4",
  },
  {
    id: 292,
    title: "Kina Chir",
    videoID: "79bAt-Fht0c",
  },
  {
    id: 293,
    title: "Hasi Ban Gaye",
    videoID: "-b1SygKSqws",
  },
  {
    id: 294,
    title: "Raataan Lambiyan",
    videoID: "T8ELL2q5kEc",
  },
  {
    id: 295,
    title: "Wajah Tum Ho",
    videoID: "l7reeXScR_U",
  },
  {
    id: 296,
    title: "Thodi Jagah",
    videoID: "jGXPLIGZS7k",
  },
  {
    id: 297,
    title: "Mere Humsafar",
    videoID: "vfz3KbJpx_E",
  },
  {
    id: 298,
    title: "Tum jo aaye",
    videoID: "azHbsWaJDUc",
  },
  {
    id: 299,
    title: "Dil Sambhal Jaa Zara",
    videoID: "uHbKAnli9DE",
  },
  {
    id: 300,
    title: "Kho Gaye",
    videoID: "-At11SGLTcs",
  },
  {
    id: 301,
    title: "Aise Kyun",
    videoID: "sX2J8NXqIUs",
  },
  {
    id: 302,
    title: "Aaj jaane Ki Zid Na Karo",
    videoID: "0qnE3BKxAj8",
  },
  {
    id: 303,
    title: "Chal Ghar Chalein",
    videoID: "sCxpmtqimtw",
  },
  {
    id: 304,
    title: "Rasiya",
    videoID: "1yiyIX0KRRQ",
  },
  {
    id: 305,
    title: "Bade Achhe Lagte Hein",
    videoID: "ucz7Yew0TKY",
  },
  {
    id: 306,
    title: "Kajra Mohabbat Wala",
    videoID: "jPFjabGEQEw",
  },
  {
    id: 307,
    title: "Abhi Na Jaao Chhod Kar",
    videoID: "AHMV_QMmqGA",
  },
  {
    id: 308,
    title: "Baarish Lete Aana",
    videoID: "kYObx4UTimQ",
  },
  {
    id: 309,
    title: "Dhokha Dhadi",
    videoID: "U9zAOeXOX9w",
  },
  {
    id: 310,
    title: "Lo Maan Liya",
    videoID: "XWuznPs9e_M",
  },
  {
    id: 311,
    title: "Maan Meri Jaan",
    videoID: "R1APP8byIL8",
  },
  {
    id: 312,
    title: "Kahani Suno",
    videoID: "zMbRuBYun5g",
  },
  {
    id: 313,
    title: "Man Bharryaa",
    videoID: "FEnXaP7S2XY",
  },
  {
    id: 314,
    title: "Hume Tumse Pyar Kitna",
    videoID: "7XC5BhtpvMs",
  },
];
var currentIndex = 0;
var currentPlayState = false;
var currentMuteState = true;
window.onYouTubeIframeAPIReady = function () {
  player = new YT.Player("player", {
    height: "390",
    width: "640",
    videoId: data[0].videoID,
    playerVars: {
      autoplay: 1,
      mute: 1,
      playsinline: 1,
      loop: 1,
      index: 0,
    },
    events: {
      onReady: onPlayerReady,
      onError: onErrorThrow,
      onStateChange: onPlayerStateChange,
    },
  });
};
function onErrorThrow(event) {
  if (event.data === 101 || event.data === 150) {
    console.log("This video " + data[currentIndex].title + " is private");
    currentIndex = (currentIndex + 1) % data.length; // Move to the next video, loop if at the end
    player.loadVideoById(data[currentIndex].videoID);
  } else {
    console.log("An error occurred: " + event.data);
  }
}
function onPlayerReady(event) {
  event.target.playVideo();

  $("#previous-video").on("click", function () {
    playPreviousVideo();
  });

  $("#next-video").on("click", function () {
    playNextVideo();
  });
}

function onPlayerStateChange(event) {
  if (event.data === YT.PlayerState.ENDED) {
    playNextVideo(); // Play the next video when the current one ends
  }
}

function playNextVideo() {
  currentIndex = (currentIndex + 1) % data.length; // Move to the next video, loop if at the end
  player.loadVideoById(data[currentIndex].videoID); // Load the new video
}

function playPreviousVideo() {
  currentIndex = (currentIndex - 1 + data.length) % data.length; // Move to the previous video, loop if at the start
  player.loadVideoById(data[currentIndex].videoID); // Load the new video
}

$(document).ready(function () {
  particlesJS.load("particles-js", "particlesjs-config.json", function () {
    console.log("callback - particles.js config loaded");
  });
  $("#playPauseIcon i").addClass("fa-solid fa-3x fa-inverse").addClass("fa-pause");

  $("#playPauseIcon").click(() => {
    $("#playPauseIcon").empty();
    $("#playPauseIcon").append("<i></i>");
    $("#playPauseIcon i").addClass("fa-solid fa-3x fa-inverse");
    if (player.getPlayerState() === YT.PlayerState.PLAYING) {
      player.pauseVideo();
      $("#playPauseIcon i").removeClass("fa-pause").addClass("fa-play");
    } else {
      player.playVideo();
      $("#playPauseIcon i").removeClass("fa-play").addClass("fa-pause");
    }
  });

  $("#muteUnmuteIcon i").addClass("fa-solid fa-3x fa-inverse").addClass("fa-volume-mute");

  // Mute/Unmute functionality
  $("#muteUnmuteIcon").click(() => {
    $("#muteUnmuteIcon").empty();
    $("#muteUnmuteIcon").append("<i></i>");
    $("#muteUnmuteIcon i").addClass("fa-solid fa-3x fa-inverse");
    if (player.isMuted()) {
      player.unMute();
      $("#muteUnmuteIcon i")
        .removeClass("fa-volume-mute")
        .addClass("fa-volume-high");
    } else {
      player.mute();
      $("#muteUnmuteIcon i")
        .removeClass("fa-volume-high")
        .addClass("fa-volume-mute");
    }
  });

  // Volume bar functionality
  $("#volume-bar").on("input", function () {
    const volume = $(this).val();
    player.setVolume(volume);
    if (volume == 0) {
      $("#muteUnmuteIcon i")
        .removeClass("fa-volume-high")
        .addClass("fa-volume-mute");
    } else {
      $("#muteUnmuteIcon i")
        .removeClass("fa-volume-mute")
        .addClass("fa-volume-high");
    }
  });
  $("#volume-bar").change(() => {
    player.unMute();
  });
  $("#nextSong").click(() => {
    playNextVideo();
  });
  $("#prevSong").click(() => {
    playPreviousVideo();
  });
});
